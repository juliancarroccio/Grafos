...
public class Rescatandoprincesaoptimo {
		...
		public Rescatandoprincesaoptimo() 
			{
			cantdragones=0;
			posprincesa=0;
			posprincipe=0;
		...
		public Rescatandoprincesaoptimo(String arg)
			{
			File file = null;
			FileReader fr = null;
		...
		public static void main(String[] args) 
			{
			Rescatandoprincesaoptimo Rescate1=new Rescatandoprincesaoptimo("C:/Users/Mutu/Documents/Programacion 3/Grafos/Rescate1.txt");
		...
		private void resolver()
			...
			this.dijkstra(this.posprincesa-1);//posicion del nodo empieza por el 0
			costoprincipe=this.costocamino(this.posprincipe-1);
			if(costoprincipe!=0xFFFF)
				{
				for(int f=0;f<this.cantdragones;f++)
					{
					aux=this.costocamino(this.posdragones[f]-1);
					if(aux<min)
						min=aux;
					}
				if(costoprincipe<min)
					{
					this.recuperarcamino(this.posprincipe-1);
